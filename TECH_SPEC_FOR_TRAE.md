...
用户在 md 中用 `[[...]]` 引用文献。`...` 部分接受两种输入：

| 输入 | 内部归一化 | 说明 |
|---|---|---|
| `[[10.1234/abc]]` | `10.1234/abc` | 纯 DOI（首选，推荐） |
| `[[https://doi.org/10.1234/abc]]` | `10.1234/abc` | DOI 链接（也接受，自动归一化） |
| `[[10.1234/abc\|Smith 2020]]` | `10.1234/abc` | 自定义显示文本（可选） |

**解析规则**：

1. 去掉方括号 `[[` `]]`
2. 如果包含 `|`（管道符），按 `|` 分割，前半是标识（DOI），后半是显示文本（可选）
3. 匹配 DOI 格式：`10\.\d{4,9}/[-._;()/:A-Z0-9]+`（不区分大小写）
4. 匹配 DOI 链接格式：`https?://(dx\.)?doi\.org/(.*)`，提取 `/` 后部分作为 DOI
5. 解析失败 → 在编辑器中标红 + 提示"非合法 DOI 或 DOI 链接"
6. 归一化后存到 `references` 表（CSL JSON 格式），DOI 是唯一键

**范例**：

```markdown
As shown in previous work [[10.1038/s41586-020-2649-2]], ...
This matches the findings of [[https://doi.org/10.1126/science.abc1669]] (Smith 2020).
Multiple citations: [[10.1234/abc]][[10.1234/def]]
```
...
| md 输入 | LaTeX 输出 |
|---|---|
| `[[10.1234/abc]]` | `\cite{doi_10_1234_abc}` |
| `[[10.1234/abc\|Smith 2020]]` | `\cite{doi_10_1234_abc}`（显示文本只在 md 渲染用） |
| `[[10.1234/abc]][[10.1234/def]]` | `\cite{doi_10_1234_abc, doi_10_1234_def}` |
| `[[10.1234/abc]]^` | `\textsuperscript{\cite{doi_10_1234_abc}}`（上标引用） |

**映射规则**：

1. DOI 中的 `/` 替换为 `_`（避免 LaTeX 解析错误）+ 字母数字下划线保留
2. 多个连续 `[[...]]` 合并成一个 `\cite{...}`，逗号分隔
3. 上标后缀 `^` → `\textsuperscript{\cite{...}}`（某些期刊要求上标引用）
4. 显示文本（`\|text`）只在 md 渲染时显示，LaTeX 引用按编号或作者年（按 CSL 模板）

### 12.3 引用排序与编号

**编号方式**：首次出现顺序（v0.2.5 §5.8 第 4 步已规定）

1. 扫描整个 md，提取所有 `[[...]]`，按出现顺序分配编号 `[1]`, `[2]`, `[3]`, ...
2. 同一 DOI 多次出现 → 复用首次编号
3. 编号格式由期刊 CSL 模板决定：
   - 默认：`[1]`, `[2]`, `[3]`
   - 作者年：`(Smith, 2020)`, `(Jones et al., 2021)`
   - 数字 + 尖括号：`<1>`, `<2>`, `<3>`
4. LaTeX 模板里的 `\cite{...}` 替换为对应编号（由 CSL 决定）

**参考文献表**（自动生成在文末）：

- 顺序：与正文首次出现顺序一致
- 格式：由 CSL 模板决定（APA / Nature / Science / ACS / IEEE 等）
- DOI 解析失败 → 参考文献表中占位 `?` + 编译时报错 `Undefined reference: doi_10_1234_abc`

### 12.4 编译流程

```
md → 解析 [[DOI]] → 归一化 → 编号分配 → 生成 references 表（CSL）
                                          ↓
                    LaTeX 模板 + 正文 + references 表 + 图片/公式/表格
                                          ↓
                       引擎链：xelatex → pdflatex → lualatex → tectonic
                                          ↓
                            失败 → _parse_latex_log 解析 → 结构化错误
                                          ↓
                            成功 → {journal-slug}-{yyyy-mm-dd-HHMM}.pdf
```

**引擎链降级**：默认 `xelatex`，失败依次尝试 `pdflatex`、`lualatex`、`tectonic`。`_parse_latex_log` 提取错误位置、错误类型、错误信息，返回结构化错误给用户。

**双产物**：

- PDF：LaTeX 编译结果
- Word：md → Pandoc（WASM 化或等价方案）→ .docx，参考文献按 CSL 生成

### 12.5 DOI 解析失败处理

| 场景 | 用户提示 | 编译行为 |
|---|---|---|
| 完全不是 DOI 格式 | 编辑器标红 + 提示"非合法 DOI 或 DOI 链接" | 阻止编译，要求修正 |
| DOI 链接格式但解析后不是合法 DOI | 编辑器标红 + 提示"DOI 链接解析失败" | 阻止编译 |
| 合法 DOI 但 CrossRef 查不到 | 警告"该 DOI 未在 CrossRef 找到，手动填写元数据" | 允许编译，参考文献用用户填的元数据 |
| 合法 DOI + CrossRef 有但网络失败 | 警告"CrossRef 暂时不可用，使用缓存元数据" | 允许编译（如果有过缓存） |

---

## §13 Trae 10 次速通施工指南

> 给 Trae 的实现抓手，按顺序施工，每步 1 次速通 = 1 轮对话。

### 13.1 施工顺序（10 步）

| 步 | 主题 | 主要章节 | 关键交付 |
|---|---|---|---|
| 1 | 项目骨架 + 首启初始化 | §7 + §2.4 | BYO 架构 + 首次初始化流程 |
| 2 | Vditor 编辑器（wysiwyg） | §5.7 + §8.1 | 7 块类型 + 图片 base64 + 1.5s 防抖 |
| 3 | 引用语法层 | §12.1 | DOI/DOI 链接双输入兼容 + 归一化 |
| 4 | 投稿编译（LaTeX） | §5.8 + §12.2-12.4 | `\cite{...}` 映射 + 引擎链 + 排序 |
| 5 | 投稿编译（Word） | §5.8 + §12.4 | Pandoc WASM 化或等价方案 |
| 6 | AI 双引擎 | §6 | AI-1 生成 + AI-2 全量审阅 |
| 7 | 学习模块（Cat 借鉴） | §8.1 + §8.2 | 3 tab + 6 题型 + SM-2 + 关键词可配 |
| 8 | 关键词追踪 | §8.1 + §8.2 | 多源（CrossRef/PubMed/arXiv/OpenAlex） |
| 9 | 数据存储 + 字段命名 | §8.2 + §2.3 | md+csv + 字段统一 |
| 10 | 集成测试 + UI 打磨 | §5 + §7 | 端到端跑通 8MB PDF 上传 + 编译 |

### 13.2 关键检查点（每步必跑）

- **步 1**：能跑 `npm run dev` 看到 BYO 初始化提示（要 GitHub PAT）
- **步 2**：Vditor 能插入 7 种块 + 图片 base64 内嵌 + 关浏览器不丢草稿
- **步 3**：输入 `[[10.1234/abc]]` 和 `[[https://doi.org/10.1234/abc]]` 都归一化成同一个 DOI
- **步 4**：编译一篇 1 页 md 出 PDF + 引用编号正确 + 引擎链降级
- **步 5**：同一篇 md 出 Word + 引用一致
- **步 6**：AI-1 生成一段摘要 → AI-2 标出 2 处问题 → 修正后通过
- **步 7**：添加 10 个词卡 → 测试 6 题型 → 复习按 SM-2 调度
- **步 8**：配置关键词组 → 抓取 4 源各 10 篇 → 失败时切源
- **步 9**：所有数据 md+csv 双格式 + 字段命名一致（`definition_cn` 不是 `def_cn`）
- **步 10**：8MB PDF 上传不超时 + 编译产物 PDF/Word 都生成 + 编译产物图片不断链

### 13.3 必读章节清单

Trae 拿到本文档后，按以下顺序读：

1. §0 定位与目标用户（30 秒，决定要不要做）
2. §1 全局硬约束（必读，违反任何一条 = 重做）
3. §2 架构总览（项目骨架的图）
4. §5.7 编辑器（步 2 实施细节）
5. §5.8 投稿编译（步 4-5 实施细节）
6. §6 AI 双引擎（步 6 实施细节）
7. §7 分发与首次初始化（步 1 实施细节）
8. §8 Cat/PA 借鉴清单（步 7-8 实施细节）
9. §9 决策记录（避免重复讨论已闭环问题）
10. **§12 引用与编译流程细节**（步 3-4 实施细节）
11. **§13 Trae 10 次速通施工指南**（施工顺序）

### 13.4 实现抓手（与 §8 对照）

| §8.1 继承 | 实现抓手 |
|---|---|
| Cat SmartEditor 7 块类型 | Vditor wysiwyg + 7 块插件（文本/图片/公式/表格/思维导图/链表/引用块） |
| Cat Learn 三 tab + 6 题型 + SM-2 | 三个路由 + 6 个组件 + SM-2 调度器 |
| Cat 单词字段基础结构 | `word, definition_cn, definition_en, example, tags, ...` |
| PaperAssistant 五阶段 | 5 个向导步骤：选题 / 文献综述 / 正文 / 引用 / 排版 |
| PaperAssistant Vditor ^3.10.4 | `package.json` 锁版本，`pnpm i vditor@^3.10.4` |
| PaperAssistant 图片 base64 内嵌 md | Vditor 上传图片 hook → 转 base64 → 写入 md |
| PaperAssistant 引擎链 | 4 引擎降级：xelatex → pdflatex → lualatex → tectonic |
| PaperAssistant _parse_latex_log | 正则解析日志，提取 `! Error` 行 + 行号 + 上下文 |
| PaperAssistant 双面板布局 | 左侧编辑 / 右侧预览，可调宽度 |

| §8.2 切断 | 实现抓手 |
|---|---|
| Cat SQLite/JSON → md+csv | 所有数据用 `.md` 或 `.csv` 落盘，不引入 SQLite |
| Cat 关键词硬编码 → 用户可配 | 关键词组存 `config/keywords.yaml`，用户可编辑 |
| Cat 追踪单源 CrossRef → 多源 | 4 源并行：`crossref` / `pubmed` / `arxiv` / `openalex` |
| Cat Math.random() 兜底 → 明示"AI 不可用" | AI 失败时显示"AI 不可用，请手动编辑"，不随机 |
| Cat 字段命名不一致 → 统一 | 全项目一个命名规范（snake_case + 后缀表义） |
| Cat 混编辑/预览分栏 → 内联 | Vditor wysiwyg 模式就是内联，不需要分栏 |
| Cat/PaperAssistant 编辑器元素弹窗输入 → 内联 | Vditor 工具栏内联插入 + 就地编辑 |
| 老 PaperAssistant 编辑器混块式/纯 md → 物理分离 | 渲染层（Vditor 实例）和 md 落盘（state.md）解耦 |
| Cat 输出 md 让用户自己转格式 → 软件交付 PDF+Word | 默认导出 PDF+Word，不再"导出 md" |
| PDF/Word 图片留断链 → 图片必须实嵌 | Pandoc 用 `--self-contained` 或等价方案 |
...
word:
  ...
  review:
    interval: <number>     # SM-2 间隔（天）
    ease: <number>          # SM-2 难度因子
    next_review: <ISO8601>  # 下次复习时间
```

命名规则：snake_case + 后缀表义（`_cn` 中文 / `_en` 英文 / `_at` 时间 / `_id` 标识符 / `_url` 链接 / `_path` 路径）。
...
│   │   ├── doi.ts          # DOI 解析 + 归一化
│   │   ├── csl.ts          # CSL 模板加载
│   │   ├── latex.ts        # md → LaTeX 映射
│   │   ├── log.ts          # _parse_latex_log
│   │   └── ai/             # AI 双引擎
│   ├── pages/              # 路由页面
│   ├── stores/             # 状态管理
│   └── main.tsx
├── public/
│   ├── templates/          # LaTeX 模板（按期刊分）
│   ├── csl/                # CSL 样式（按期刊分）
│   └── tectonic/           # Tectonic WASM
├── config/
│   └── keywords.yaml       # 关键词组（用户可配）
├── docs/
│   └── decisions/          # 决策记录
├── package.json
├── TECH_SPEC.md            # ← v0.2.5 定稿归档
└── TECH_SPEC_FOR_TRAE.md   # ← 你正在读
```
...
**给 Rosa**：把仓库根 `/TRAE_PROMPT.md` 的内容复制粘贴到 Trae IDE AI 对话框即可。

**给 Trae**：见 `/TRAE_PROMPT.md`，那是给你的完整指令。
