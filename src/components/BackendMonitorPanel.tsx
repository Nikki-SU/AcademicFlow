    ...
    <div className="space-y-2">
      {tasks.map((t: any) => {
        ...
        return (
          <div
            key={t.id}
            className="border border-slate-200 rounded-lg p-2.5 bg-white hover:border-slate-300 transition"
          >
            <div className="flex items-center gap-2 mb-1.5">
              <FourNodeBar nodeIndex={t.node_index ?? 0} status={t.status} />
              <span className="flex-1 min-w-0 text-xs font-medium text-slate-700 truncate" title={t.title}>
                {t.title}
              </span>
              <span
            ...
            <div className="flex items-center gap-1.5 text-[11px] text-slate-500">
              ...
              {t.status === 'aborted' && <XCircle className="w-3 h-3 text-slate-400" />}
              ...
              {t.status === 'pending' && <Clock className="w-3 h-3 text-slate-400" />}
  ...
  return (
    <div className="border border-slate-200 rounded-lg overflow-hidden bg-white">
      ...
      >
        ...
        {trace.totalMs != null && (
          <span className="text-[10px] text-slate-400 shrink-0">{(trace.totalMs / 1000).toFixed(1)}s</span>
        )}
      </button>
      {open && (
        <div className="border-t border-slate-100 bg-slate-50 px-2.5 py-2 text-[11px] space-y-1">
          {trace.stages.length === 0 && (
            <div className="text-slate-400 italic">暂无阶段记录</div>
          )}
...
function StageRow({ s }: { s: StageTrace }) {
  const engineDot = s.aiEngine === 'ai1' ? 'bg-purple-400' : s.aiEngine === 'ai2' ? 'bg-blue-400' : s.aiEngine === 'code' ? 'bg-green-400' : 'bg-slate-300'
  return (
    <div className="flex items-center gap-2">
      <span className={`inline-block w-1.5 h-1.5 rounded-full ${engineDot} shrink-0`} />
      <span className="text-slate-600 flex-1 min-w-0 truncate" title={s.label}>
        {s.label}
      </span>
      {s.durationMs != null && (
        <span className="text-slate-400 shrink-0">{(s.durationMs / 1000).toFixed(1)}s</span>
      )}
    ...
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="px-3 py-2.5 bg-slate-50 border-b border-slate-200">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-sm text-slate-800">后台监控</span>
          {runningCount > 0 && (
          ...
          {pendingCount > 0 && (
            <span className="text-[10px] text-slate-500">· {pendingCount} 排队</span>
          )}
        </div>
      </div>
