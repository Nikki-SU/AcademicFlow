# AcademicFlow Worker（MinerU 透传代理）

MinerU HTTP API 的**透传代理**（无 CORS 头 → 浏览器直连挂）。同一份代码，两种 Runtime。

代理代码位于 AcademicFlow 仓库的 `worker/` 子目录。
...
| **入口文件** | `worker/deno.js` | `worker/worker.js` |
| **推荐给** | 国内用户（默认） | 有代理 / 出海用户 |

两种方案都调用同一个 `worker/core.js` 处理请求，功能 100% 等价，只差在跑在谁的机房。选一个部署即可。

---

## 方案 A：Deno Deploy 部署（国内用户，默认）

1. Fork `https://github.com/Nikki-SU/AcademicFlow`
2. 打开 <https://dash.deno.com/new>
3. **Sign in with GitHub** → 授权 Deno Deploy 读取仓库
4. **Deploy from GitHub repository** → 搜索你 fork 的 `AcademicFlow` → 选中
   - **Entrypoint**：填 `worker/deno.js`
   - **Install Step**：留空
   - **Build Step**：留空
5. 点 **Deploy Project** → 得到 `https://<项目名>.deno.dev` URL

粘回 AcademicFlow → Settings → MinerU 代理 输入框。前端自动 ping `/__af_health`，绿色 ✓ 就是好了。
...
1. Fork `https://github.com/Nikki-SU/AcademicFlow`
2. 点 [Deploy to Cloudflare Workers](https://deploy.workers.cloudflare.com/?url=https://github.com/Nikki-SU/AcademicFlow) → CF 登录
3. 页面上直接点 **Deploy**，全程 CF 官方引导，无自定义步骤
4. 得到 `https://<项目名>.<你的用户名>.workers.dev` URL

粘回 AcademicFlow → Settings → MinerU 代理 输入框。

> ⚠️ `*.workers.dev` 域名在国内三大运营商直连普遍不稳，若你在国内且没代理，请改用方案 A。

---

## 方案 C：VPS 一键部署（国内用户，推荐）

在你的 VPS 上执行：
```bash
curl -fsSL https://nikki-su.github.io/AcademicFlow/install.sh | bash
```

脚本会自动安装 Deno、克隆仓库到 `/opt/academicflow`、配置 systemd 开机自启服务。
